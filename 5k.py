...
wait = {
    ...
    "Respontag":"🌺 SELFBOT-BY:TANBOTMEVERDIE✯͜͡❂➣ 🌺",
    "welcome":"Selamat datang & semoga betah",
    "comment":"Like like & like by SELFBOT-BY:TANBOTMEVERDIE✯͜͡❂➣",
    "message":"บัญชีนี้ได้รับการป้องกันโดย\n🇹🇭 SELFBOT-BY:TANBOTMEVERDIE✯͜͡❂➣ 🇹🇭\nทางบัญชีจึงขอทำการบล็อค\nเพื่อความปลอดภัยในบัญชี 💕",
    }
...
def sendMention(to, mid, firstmessage):
    try:
        ...
        bot = runtime(eltime)
        text += mention+"◐ Jam : "+datetime.strftime(timeNow,'%H:%M:%S')+" Wib\n🤖 Group : "+str(len(gid))+"\n🤖 Teman : "+str(len(teman))+"\n🤖 Expired : In "+hari+"\n🤖 Version : SELFBOT-BYTANBOTMEVERDIE✯͜͡❂➣:\n🤖 Tanggal : "+datetime.strftime(timeNow,'%Y-%m-%d')+"\n🤖 Runtime : \n • "+bot
               ...
               if msg.contentType == 1:
                   if msg._from in admin:
                       if mid in Setmain["ptatan1983foto"]:
                            path = cl.downloadObjectMsg(msg_id)
                            del Setmain["ptatan1983foto"][mid]
               ...
               if msg.contentType == 1:
                 if msg._from in admin:
                        if Amid in Setmain["ptatan1983foto"]:
                            ...
                            del Setmain["ptatan1983Sfoto"][Bmid]
               ...
               if msg.contentType == 0:
                    ...
                    if text is None:
                        ...
                        if cmd == "self on":
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               sendMention(msg.to, sender, "「 SELFBOT-BY:TANBOTMEVERDIE✯͜͡❂➣ 」\n")
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               ...
                               for group in saya:
                                   cl.sendMessage(group,"[ SELFBOT-BY:TANBOTMEVERDIE✯͜͡❂➣ ]\n" + str(pesan))
                          ...
                          if wait["selfbot"] == True:
                              if settings["autoJoinTicket"] == True:
                                 ...
                                 for ticket_id in n_links:
                                     ...
                                     cl.sendMessage(msg.to, "🤖 TANBOTMEVERDIE✯͜͡❂➣:MAX 🤖 : %s" % str(group.name))
                                     group1 = ki.findGroupByTicket(ticket_id)
                                     ki.acceptGroupInvitationByTicket(group1.id,ticket_id)
                                     ki.sendMessage(msg.to, "🤖 SELFBOT-BY:TANBOTMEVERDIE✯͜͡❂➣ 🤖 : %s" % str(group.name))
                                     group2 = kk.findGroupByTicket(ticket_id)
                                     kk.acceptGroupInvitationByTicket(group2.id,ticket_id)
                                     kk.sendMessage(msg.to, "🤖 SELFBOT-BY:TANBOTMEVERDIE✯͜͡❂➣ 🤖 : %s" % str(group.name))
                                     group3 = kc.findGroupByTicket(ticket_id)
                                     kc.acceptGroupInvitationByTicket(group3.id,ticket_id)
                                     kc.sendMessage(msg.to, "🤖 SELFBOT-BY:TANBOTMEVERDIE✯͜͡❂➣ 🤖 : %s" % str(group.name))
                                     group4 = kb.findGroupByTicket(ticket_id)
                                     kb.acceptGroupInvitationByTicket(group4.id,ticket_id)
                                     kb.sendMessage(msg.to, "🤖 SELFBOT-BY:TANBOTMEVERDIE✯͜͡❂➣ 🤖 : %s" % str(group.name))
                                     group5 = kd.findGroupByTicket(ticket_id)
                                     kd.acceptGroupInvitationByTicket(group5.id,ticket_id)
                                     kd.sendMessage(msg.to, "🤖 SELFBOT-BY:TANBOTMEVERDIE✯͜͡❂➣ 🤖 : %s" % str(group.name))
